...
public class CustomerController {
    ...
    public ModelAndView createCustomerForm(@Validated @ModelAttribute("customerDTO") CustomerDTO customerDTO, BindingResult bindingResult) {
        ...
        if(customerService.existsByFullName(name)) {
            bindingResult.addError(new ObjectError("nameError", "Name is invalid! Name cannot be left blank!"));
        }

        if (customerService.existsByPhone(phone)) {
            bindingResult.addError(new ObjectError("phoneExists", "Phone is invalid or already exists!"));
        }

        if (customerService.existsByEmail(email)) {
            bindingResult.addError(new ObjectError("emailExists", "Email is invalid or already exists!"));
        }
