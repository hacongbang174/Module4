        ...
        if (ParsingValidationUtils.isLongParsable(transfer)) {
            ...
            if (errors.isEmpty()) {
                ...
                if (customerSender.getBalance().compareTo(BigDecimal.valueOf(transaction_amount)) < 0) {
                    errors.add("SCurrent balance is not enough. Please withdraw smaller amount " + customerSender.getBalance().subtract(BigDecimal.valueOf(fees_amount)));
                } else {
                    ...
                    if (balanceRecipent.toString().length() > 12) {
                        errors.add("Exceeded the total amount of the recipient. The total amount of the recipient is less than 12 digits.");
                    } else {
